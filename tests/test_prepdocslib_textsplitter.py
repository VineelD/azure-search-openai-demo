...
async def test_sentencetextsplitter_list_parse_and_split(tmp_path, snapshot):
    ...
    for f in Path("data").glob("*.md"):
        shutil.copy(str(f.absolute()), tmp_path)
    ...
    async for file in files:
        pages = [page async for page in text_parser.parse(content=file.content)]
        assert pages
        sections = [
...
def pytest_generate_tests(metafunc):
    """Parametrize the test_doc fixture with text files in the test-data directory."""
    if "test_doc" in metafunc.fixturenames:
        text_files = list(Path("tests", "test-data").glob("*.txt"))
        metafunc.parametrize("test_doc", text_files if text_files else [None])


@pytest.mark.asyncio
async def test_sentencetextsplitter_multilang(test_doc, tmp_path):
    if test_doc is None:
        pytest.skip("No .txt files in tests/test-data")
    text_splitter = SentenceTextSplitter()
